...
# Goal: Evaluate dung beetle diversity trends over forest gradient separately for winners and lsoers
# 1. Clean data
# 2. Calculate N, TD, FD, PD of winners and losers per trap
# 3. Select modeling spatial scale
# 4. Model how the diversity of winners and losers changes in response to longitude, total edge, and % forest cover
# 5. Model how the diversity of winners and losers differs on either side of a forested barrier
# 6. Generate graphics
...
# This dataset was generated in Brazil, so occasionally variable names are in Portuguese.
...
# NFS = non-forest specialist
# P = pasture
# M = mata (forest)
# C = cerrado
# MA = mata atlantica (atlantic forest)

#########################
# Set directory         
#########################

setwd("/Users/Tyler/Dropbox/Interface/Dados/Non-Geospatial/Biodiversity/Analysis/Data") 

# Also set folders to write datafiles and figures to
results = "~/Dropbox/Interface/Dados/Non-Geospatial/Biodiversity/Analysis/Data/results" 
figures = "~/Dropbox/Interface/Dados/Non-Geospatial/Biodiversity/Analysis/Data/figures" 
...
make.composite.with.polytomies.2 <- function(tree, genera, species, max.genus.age=NA){
  ...
  #A hack to fix the ultrametric nature	
...
# load in data file
load("Biodiversity.RData")

# Contents:
# df = data frame with captures of each species at each point, and information about each point
# traits.df = data frame with trait data for each species, contained within a larger dung beetle trait database
# base.phy = dung beetle phylogeny from Monaghan et al. 
# code.df = data table from Monaghan et al., allowing us to match code in phylogeny to genus, species
# buffer.df = landscape composition data
# land.config = landscape configuration data
...
###############################################
# get rid of observations that are 0 because they are missing
###############################################
...
# # If you are going to run this code more than once, write these results out as a csv, and next time just read it in using the code below instead of running the above.
...
# # Do this to save time if running the code more than once
...
# extract 200m (landscape-scale, selected in the step above) FC
...
FD.ses.NFS$FDses_stand = (FD.ses.NFS$FDses_norm * (length(FD.ses.NFS$FDses_norm) - 1) + 0.5) / length(FD.ses.NFS$FDses_norm) # this step allows us to run the beta regression by nudging the extremes away from 0 and 1
FD.test.NFS <- NULL
...
############################################
# Table 1. write out a table of model coefficients
#########################################
...
aic.names = c("Metric", "Classification","Variable", "Z-value")
colnames(table) <- aic.names

# round z-value
table[,4] <-formatC(round(as.numeric(table[,4]), 2), format = 'f', digits = 2)
...
# Wilcoxon signed rank test: test difference between N for FS and NFS
# doesn't assume normality or equality of variance, does use paired samples
# replace missing FS and NFS observations at certain points with 0s 
model.df.final.t <- model.df.final
model.df.final.t[is.na(model.df.final.t)] <- 0
wilcox.test(model.df.final.t$TD_FS_abund, model.df.final.t$TD_NFS_abund, paired = TRUE)
# V = 1351.5, p-value = 0.01161
...
# Wilcoxon signed rank test: test difference between N for FS and NFS
wilcox.test(model.df.final.t$TD_FS_raw, model.df.final.t$TD_NFS_raw, paired = TRUE)
# V = 1074.5, p-value = 0.08803
...
# buffered landscapes, "eastern": 148, 266, 291, 317
# exposed landscapes, "western": 215, 263, 282, 329, 333, 335, 359, 399
...
########################################################
# Table 2. write out a table of permutation test results
########################################################
...
#####################################################################
# Binned Banks-Leite matrix for % FC
####################################################################
...
# add in empty bins, including one at beginning for graphing ease (see list of empty bins below)
...
# this section only runs when FD section of the code has run
...
###############################
# Plot N vs. long for each species 
############################
...
# Set up function to calculate phylogenetic signal for categorical traits
...
# Calculate phylogenetic signal for diel
result<-phylo.signal.disc(trait.temp3$Diel,species.phy,cost=NULL)
summary(result) # p = .999

# Calculate phylogenetic signal for nesting strategy
result<-phylo.signal.disc(trait.temp3$Nest,species.phy,cost=NULL)
summary(result) # p = .999


# Use blomberg's k to calculate phylogenetic signal for continuous traits
