...
b.bdv.df2 = read_excel("beetle_biodiversity_27_09_16.xlsx", 
...
b.tr.df1 = read_excel("beetle_species_traits_23_09_16.xlsx", sheet = 1, 
                      col_names = TRUE)
...
# rename Code_min col name to TrapID
names(b.fc.df)[names(b.fc.df) == "Code_min"] <- "TrapID"
...
b.fc.df$TrapID <- as.character(b.fc.df$TrapID)
b.fc.df$TrapID_min <- substr(b.fc.df$TrapID, 1, 8)
...
b.bdv.df_melt = melt(b.bdv.df, id.vars=c(1:16,79), measure.vars=17:78, 
                     variable_name="code")
...
# for each species within each point, 
# sum up N over all traps, and stores in 'N.trap' column
b.sum.df = ddply(b.bdv.df_melt1, 
                 .(Landscape, Point, code, TrapID_min), 
                 summarize, N = sum(N.trap), 
                 Lat = mean(Lat), Long = mean(Long))

# determine presence/absence for a species at a point 
# and assigns 1 (present) or 0 (absent) to 'presence' column
...
# merge b.tr.bdf.df with percent forest cover per point data 
...
# rename columns appropriately 
b.df <- reshape::rename ( b.df, replace = c ("code" = "Species" , 
                                             "presence" = "Presence" , 
                                             "Biogeo_assoc" = "Biogeo" , 
                                             "BM_mean_mg" = "BM" , 
                                             "Hab_pref" = "Hab" ))
...
# determine which species were captured fewer 
#than three times across all landscapes
...
# determine the max number of caps a point per species

### export final b.df object to data directory as binary data object (.rds)
saveRDS ( b.df , file = "../Intermediates/b.df.rds" ) 
