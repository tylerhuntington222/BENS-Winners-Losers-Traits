...
# PURPOSE: Aggregate, clean and organize beetle data for WLT analyses.

# INPUT: Various data files in raw data files in Data directory.

# OUTPUT: cleaned and consolidated beetle data in .rds binary file format
#         exported to Intermediate_output directory as `b.df.rds`
...
a.bdv.df = data.frame(read_excel("Bird_species_abundance_18_10_16.xlsx", 
                                 sheet = 1, col_names = TRUE ))

# read avian trait data
a.tr.df1 = data.frame(read_excel( "Bird_species_traits_21_11_16.xlsx" , 
                                  sheet = 1 , col_names = TRUE ))

# read avian biogeographic association data from bird encyclo
a.biogeo.df1 <- data.frame(read_excel ("avian_encyclopedia_19_1_17.xls", 
                                       sheet = 1, col_names = TRUE))
...
TrapID.key.df<- data.frame(read_excel("Trap_Code_Key.xlsx", 
                                      sheet = 1, col_names = T))

# read avian FC data
a.landcomp.df <- read.csv("Landscape_composition_pitfalls.csv", 
                          header = T)
...
a.bdv.df_melt <- melt ( a.bdv.df , id.vars = c ( 1:5 ) , 
                        measure.vars = 9:232 , variable_name = "code" , 
                        header = TRUE )
...
# for each species within each point, sum up N over all 
# traps and write to new column "N"
a.sum.df = ddply (a.bdv.df_melt , 
                  .( Landscape , Habitat , Point , code , TrapID_min ) , 
                  summarize , N = sum ( N.count ) )

#  creates a presence/absence column per species per point and determines 
# presence/absence for all species/point [s] and 
...
b.cap.summary <- ddply ( a.sum.fcaps.df, .( code ) , 
...
# # Identify which avian species are in a.tr.df but not a.sum.fcaps.df 
# (i.e. non-overlap between dfs)
...
a.biogeo.df <- reshape::rename (a.biogeo.df1, 
                                replace = c("code" = "species.genus"))

#subset relevant columns of a.biogeo.df
a.biogeo.df <- subset ( a.biogeo.df, 
                        select = c("species.genus" , "NHAB" , 
                                   "NZOO" , "biogeo_assoc.v1"))
...
#all.spec.df <- ddply (a.df, .(a.df$species.genus, a.df$biogeo_assoc), 
# summarize, bm = mean (Point) )

# rename NHAB and NZOO columns in a.df
a.df <- reshape::rename (a.df, 
                         replace = c("NHAB" = "hab_plasticity" , 
                                     "NZOO"= "biogeo_plasticity", 
...
a.df <- reshape::rename ( a.df, replace = c 
                          ("TrapID_min" = "TrapID" , 
                            ...
                            "Nesting" = "Nest" , 
                            "biogeo_assoc" = "Biogeo" , 
                            "biogeo_plasticity" = "BiogeoPlas" , 
                            "Body.size.g" = "BM" , 
                            "hab_assoc" = "Habitat" , 
                            "Eggs_minmax_mean" = "Fecund" , 
                            "diet_plasticity" = "DietPlas" , 
                            "hab_plasticity" = "HabPlas" , 
                            "diet_primary" = "Diet"))
...
# determine which species were captured fewer than three times in all landscapes
...
saveRDS ( a.df , file = "../Intermediates/a.df.rds" )
