...
# 2) Create empty buffers at 800m radius around each point
...
setwd("/Users/nicholslab/Dropbox/Interface/Dados/Non-Geospatial/Winners_Losers_Traits/Analysis/Data/Bird_Sampling_Points")
...
a.pts.geo1 <- readOGR("/Users/nicholslab/Dropbox/Interface/Dados/Non-Geospatial/Winners_Losers_Traits/Analysis/Data/Bird_Sampling_Points/Point_counts_Cantareira.shp", "Point_counts_Cantareira")

# load basemap of collated landscapes
all.ls.merge1 <- readOGR("/Users/nicholslab/Dropbox/Interface/Dados/Geospatial/Private_LEPAC/Final products/Mapeamento/merged_land_use/all_landscapes_merge.shp" , "all_landscapes_merge")
...
# create buffer polygons of 800m radius around each point
a.buff <- gBuffer(a.pts, width=800, byid=TRUE, quadsegs=100)
...
# load merged avian sampling point 800m buffers intersected universal LC basemap

a.all.b800.LC.insct <- readOGR("/Users/nicholslab/Dropbox/Interface/Dados/Non-Geospatial/Winners_Losers_Traits/Analysis/Data/Bird_Sampling_Points/a.allpts_insct" , "a.allpts_insct")

a.all.b800.LC.insct$ponto <- gsub (" ", ".",a.all.b800.LC.insct$ponto, fixed = TRUE) 

a.all.b800.LC.insct$ponto

# Create vector containing point LS codes

a.pt.LS.codes <- (a.all.b800.LC.insct$ponto)
...
for (i in a.pt.LS.codes.v) {
  ...
  writeOGR(subset (a.all.b800.LC.insct, a.all.b800.LC.insct$ponto == i), dsn = paste("a.", i ,".b.800.LC", sep = ""), layer = paste("a." , i ,".b.800.LC", sep = ""), driver = "ESRI Shapefile")
...
for (pt in a.pt.LS.codes.v) {
  ...
  point = readOGR(dsn = paste("/Users/nicholslab/Dropbox/Interface/Dados/Non-Geospatial/Winners_Losers_Traits/Analysis/Data/a.pts.b800.LC.shapes/a.", pt, ".b.800.LC/a." , pt ,".b.800.LC.shp" , sep = ""), layer = paste ("a." , pt , ".b.800.LC" , sep = ""))
  ...
  writeOGR(spdf, dsn = paste(pt, ".b800.dissolve", sep = ""), layer = paste(pt, ".b800.dissolve", sep = ""), driver = "ESRI Shapefile")
}
