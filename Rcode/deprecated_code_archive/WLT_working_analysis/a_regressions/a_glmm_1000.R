...
################################################ Plot N ~ FC @ 1000m for all each beetle species
...
for (i in a.sp.df$Species) {
  ...
  plot(a.par.df$perFC_1000, jitter(a.par.df$N, 0.5), main = i)
...
a.1000respFC.df <- data.frame(
  ...
  Inter.p.val = double(), 
  a.slope_1000 = double(),
  a.slope_1000.error = double (),
  a.slope_1000.t.val = double(),
  a.slope_1000.p.val = double()
)
...
levels(a.1000respFC.df$Species) <- unique (a.sp.df$Species)
...
for (i in a.sp.df$Species) {
  ...
  a.1000respFC.df[row, 1] <- (i)
  a.1000respFC.df[row,2:5] <- summary (a.glmm.par)$coefficients[1,]
  a.1000respFC.df[row,6:9] <- summary (a.glmm.par)$coefficients[2,]
...
a.1000respFC.df$a.slope_1000.p.val <- round(a.1000respFC.df$a.slope_1000.p.val, 3)
# 
# # create histogram of responses
# hist (a.1000respFC.df$a.slope_1000, breaks = 20)
# 
# # subset for species with significant a.slope_1000 coefficients
# a.sigResp <- subset (a.1000respFC.df, a.1000respFC.df$a.slope_1000.p.val < 0.05)
# 
# hist (a.sigResp$a.slope_1000, breaks = 15)
# hist (a.sigResp$a.slope_1000.p.val, breaks = 15)
# 
# 
# ### Export summary of betas generated by this modeling approach
# 
# # subset a.1000respFC.df for a.slope_1000s and p-values
# a.1000respFC.df_betas <- subset(a.1000respFC.df, select = c("Species", "a.slope_1000", "a.slope_1000.p.val"))
# 
# # rename columns to specify modeling approach number
# colnames(b.respFC.df_betas) = c("Species", "a_glmm_1000_a.slope_1000", "b_glmm_1000_p.val")
# 
# #  save as RDS object
...
# 
a.sp.df <- merge(a.1000respFC.df, a.sp.df)


#### Create preliminary traits GLM without accounting for phylo
a_trait_resp_glm <- glm(a.slope_1000 ~ BM + Biogeo + BiogeoPlas + Fecund + DietPlas + Diet + Habitat + HabPlas, data = a.sp.df)


options('na.action' = na.fail)
a_1000_trait_resp_glm.table <- dredge(a_trait_resp_glm, beta = c("none", "sd", "partial.sd"), evaluate = TRUE,
...
# alternative method for calculating consensus tree -- VERY long runtime
...
a.comp_1000 <- comparative.data(a.tre, a.sp.df, species.genus, vcv=TRUE, force.root = T)

# build pgls model
a_glmm_1000.pgls <- pgls(a.slope_1000 ~ BM + Diet + DietPlas + Nest + Biogeo + BiogeoPlas + Fecund + Habitat + HabPlas, a.comp_1000)

# generate all possible models and rank by AIC
a_pgls_1000_mods.table <- dredge(a_glmm_1000.pgls, beta = c("none", "sd", "partial.sd"), evaluate = TRUE,
                                rank = "AICc", fixed = NULL)

print (a_pgls_1000_mods.table)

summary(a_pgls_1000_mods.table)
