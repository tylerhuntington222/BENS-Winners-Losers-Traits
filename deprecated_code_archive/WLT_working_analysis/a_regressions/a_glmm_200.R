...
################################################ Plot N ~ FC @ 200m for all each beetle species
...
for (i in a.sp.df$Species) {
  ...
  plot(a.par.df$perFC_200, jitter(a.par.df$N, 0.5), main = i)
...
a.200respFC.df <- data.frame(
  ...
  Inter.p.val = double(), 
  a.slope_200 = double(),
  a.slope_200.error = double (),
  a.slope_200.t.val = double(),
  a.slope_200.p.val = double()
)
...
levels(a.200respFC.df$Species) <- unique (a.sp.df$Species)
...
for (i in a.sp.df$Species) {
  ...
  a.200respFC.df[row, 1] <- (i)
  a.200respFC.df[row,2:5] <- summary (a.glmm.par)$coefficients[1,]
  a.200respFC.df[row,6:9] <- summary (a.glmm.par)$coefficients[2,]
...
a.200respFC.df$a.slope_200.p.val <- round(a.200respFC.df$a.slope_200.p.val, 3)
# 
# # create histogram of responses
# hist (a.200respFC.df$a.slope_200, breaks = 20)
# 
# # subset for species with significant a.slope_200 coefficients
# a.sigResp <- subset (a.200respFC.df, a.200respFC.df$a.slope_200.p.val < 0.05)
# 
# hist (a.sigResp$a.slope_200, breaks = 15)
# hist (a.sigResp$a.slope_200.p.val, breaks = 15)
# 
# 
# ### Export summary of betas generated by this modeling approach
# 
# # subset a.200respFC.df for a.slope_200s and p-values
# a.200respFC.df_betas <- subset(a.200respFC.df, select = c("Species", "a.slope_200", "a.slope_200.p.val"))
# 
# # rename columns to specify modeling approach number
# colnames(b.respFC.df_betas) = c("Species", "a_glmm_200_a.slope_200", "b_glmm_200_p.val")
# 
# #  save as RDS object
...
# 
a.sp.df <- merge(a.200respFC.df, a.sp.df)


#### Create preliminary traits GLM without accounting for phylo
a_trait_resp_glm <- glm(a.slope_200 ~ BM + Biogeo + BiogeoPlas + Fecund + DietPlas + Diet + Habitat + HabPlas, data = a.sp.df)


options('na.action' = na.fail)
a_200_trait_resp_glm.table <- dredge(a_trait_resp_glm, beta = c("none", "sd", "partial.sd"), evaluate = TRUE,
...
a.comp_200 <- comparative.data(a.tre, a.sp.df, species.genus, vcv=TRUE, force.root = T)

# build pgls model
a_glmm_200.pgls <- pgls(a.slope_200 ~ BM + Diet + DietPlas + Nest + Biogeo + BiogeoPlas + Fecund + Habitat + HabPlas, a.comp_200)

# generate all possible models and rank by AIC
a_pgls_200_mods.table <- dredge(a_glmm_200.pgls, beta = c("none", "sd", "partial.sd"), evaluate = TRUE,
                                rank = "AICc", fixed = NULL)

print (a_pgls_200_mods.table[1:10,])

summary(a_pgls_200_mods.table)


# make plots of relationships of interest
plot( a.sp.df$HabPlas, a.sp.df$a.slope_200)

plot (a.sp.df$BiogeoPlas, a.sp.df$a.slope_200)
